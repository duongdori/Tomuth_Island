...
public class BoarManager : MonoBehaviour
{
    ...
    void ListAnimalControllerActive(int indexPosition)
    {
        ...
        {
            BoarController animalController = animals[i].GetComponent<BoarController>();
    ...
    // Set Model for Enemy
    public EnemyBoar GetModel(int index)
    {
        AnimalPositions animalPositions = index == 0 ? AnimalPositions.Lead : AnimalPositions.None;
        EnemyBoar enemyAnimal = new();
        if (enemyAnimal != null)
        {
            enemyAnimal.SetAnimalPositions(animalPositions);
            int level = enemyAnimal.GetAnimalPositions().Equals(AnimalPositions.Lead) ? 5 * (positionArea + 1) : Random.Range(1, 3) * (positionArea + 1);
            ...
            enemyAnimal.SetModel(baseFunction.GenerateRandomKey(), nameof(EnemyBoar), level, STR, INT, VIT, AGI, DEX);
        }
        return enemyAnimal;
    }
