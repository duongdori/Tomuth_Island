...
public class BoarController : AnimalController
{
    [SerializeField]
    Transform targetAnimalLead;
    BoarManager animalManager;
    ...
    int indexAnimal;
    EnemyBoar animalModel;
    int indexControll = 5;

    // Animation states
    const string ANIMAL = "Boar";
    string currentState = "";
    bool isIdle = true;
    ...
    void Awake()
    {
        //animalManager = FindAnyObjectByType<BoarManager>();
        animalManager = GetComponentInParent<BoarManager>();
    }
    ...
    void Update()
    {
        ...
        {
            if (stateInfo.normalizedTime >= 1.3f)
            {
                animalModel = animalManager.GetModel(indexAnimal);
    ...
    public EnemyBoar GetModelAnimal()
    {
        return animalModel;
    }
    public void SetModelAnimal(EnemyBoar animalModel)
    {
