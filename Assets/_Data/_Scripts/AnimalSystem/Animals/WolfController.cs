...
public class WolfController : AnimalController
{
    ...
    float stoppingDistance = 1.5f; // Distance at which the enemy stops near the player
    // [SerializeField]
    ...
    void Awake()
    {
        ...
        animalManager = GetComponentInParent<WolfManager>();
    }
    ...
    void Update()
    {
                ...
                if (stateInfo.IsName(baseCurrent.GetAttack(ANIMAL)) && stateInfo.normalizedTime >= 1.0f && !isHit)
                {
                    if (player != null)
                    {
                        ...
                        if (playerController != null)
                        {
                            ...
                            Vector3 leaveDestination = RandomLeaveDestination(transform.position);
                            if (baseFunction.IsValidDestination(leaveDestination))
                            {
                                MoveTowardsPlayer(leaveDestination);
                            }
                        }
                    }
                }
        ...
        {
            if (stateInfo.normalizedTime >= 2.5f)
            {
                animalModel = animalManager.GetModel(indexAnimal);
    ...
    //     float nearestDistance = 25f;
    //     Vector3 nearestPlayerPosition = endPoint.position;

    //     foreach (GameObject playerObject in players)
    //     {
    //         Vector3 distance = playerObject.transform.position - transform.position;
    //         if (distance.magnitude > nearestDistance && animalModel.GetAnimalPositions().Equals(AnimalPositions.Lead))
    //         {
    //             animalManager.ChangeStatusAnimalGroup(AnimalStatus.Peace);
    //             AnimalMovement(endPoint.position);
    //         }
    //         else
    //         {
    //             nearestPlayerPosition = playerObject.transform.position;
    //         }
    //     }
    ...
    void DetectPlayer()
    {
        ...
        // {
        //     player = hitPlayers[0].transform; // Set the player as the target
        //     isPlayerInRange = true;
        // }
        // else
        // {
    ...
    Vector3 RandomLeaveDestination(Vector3 point)
    {
        ...
        float randomX = Random.Range(-point.x / 1.25f, point.x / 1.25f);
        float randomZ = Random.Range(-point.z / 1.25f, point.z / 1.25f);
    ...
    void TargetPlayer()
    {
        ...
        // Vector3 toPlayer = PlayerController.Instance.transform.position - wolfPosition;
        // toPlayer.y = 0;
        // if (toPlayer.magnitude <= detectionRadius)
        // {
        //     if (Vector3.Dot(toPlayer.normalized, transform.forward) > Mathf.Cos(detectionRadius * 3f * Mathf.Deg2Rad))
        //     {
        //         wolfManager.ChangeStatusWolfGroup(publicKey, WolfStatus.Attack);
        //     }
        // }
        ...
        // foreach (GameObject playerObject in players)
        // {
        //     Vector3 toPlayer = playerObject.transform.position - animalPosition;
        //     toPlayer.y = 0;
        //     if (toPlayer.magnitude <= detectionRadius)
        //     {
        //         if (Vector3.Dot(toPlayer.normalized, transform.forward) > Mathf.Cos(detectionRadius * 3f * Mathf.Deg2Rad))
        //         {
        //             wolfManager.ChangeStatusAnimalGroup(AnimalStatus.Attack);
        //         }
        //     }
        // }
    ...
    public EnemyWolf GetModelAnimal()
    {
        return animalModel;
    }
    public void SetModelAnimal(EnemyWolf animalModel)
    {
